...
## Formatter
Justd utilizes the <NewTab href='https://biomejs.dev/'>Biome</NewTab> formatter for code formatting. To use the Biome formatter with Justd components, we recommend the following configuration:
...
Following that, include the **Tailwind Animate** for managing animations and **Justd Icons** for icons:

<Installation options={{isManual: true}} items={['justd-icons', 'tw-animate-css']} />
...
Hooks are only used in specific components like the **Navbar**, **Sidebar**, **Popover**, and **Command Menu**. If you're using the **Justd CLI**, it will handle this automatically; otherwise, download the file below.
